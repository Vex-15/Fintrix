...
def _check_fee_change(
    ...
    for txn in related.get("transactions", []):
        ...
        for rate in settings.plausible_mdr_rates:
            ...
            if amt_score < 0.3:
                # Simple check: Does actual rate match a plausible rate?
                if abs(actual_rate - rate) < 0.001:
                    # We found a plausible historical rate that perfectly explains the fee
                    # Score based on how far it is from the expected rate
                    expected_at_2pct = round(txn_amount * settings.expected_mdr_rate)
                    confidence = min(0.95, max(0.6, 0.7 + (1.0 - abs(actual_rate - settings.expected_mdr_rate) / 0.01) * 0.2))
                    ...
                    return Hypothesis(
                        ...
                        root_cause=f"Fee discrepancy on {txn['id']}: recorded fee {recorded_fee} paise "
                                   ...
                                   f"at standard {settings.expected_mdr_rate:.1%} MDR.",
                        evidence=[
                            ...
                            f"Effective MDR rate: {actual_rate:.4%} (expected: {settings.expected_mdr_rate:.1%})",
                        ],
                        recommended_action=_determine_action(confidence, discrepancy, "fee_change"),
                        details={"actual_rate": actual_rate, "expected_rate": settings.expected_mdr_rate},
                    )
                continue
        ...
        if recorded is not None and expected is not None:
            ...
            return Hypothesis(
                ...
                root_cause=f"Fee discrepancy on {txn_id}: recorded {recorded} vs expected {expected} paise "
                           f"(effective rate {actual_rate:.3%} vs {settings.expected_mdr_rate:.1%}).",
                evidence=[
